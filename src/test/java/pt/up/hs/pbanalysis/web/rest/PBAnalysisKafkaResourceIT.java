...
import pt.up.hs.pbanalysis.config.KafkaProperties;
...
class PBAnalysisKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/pbanalysis-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/pbanalysis-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
